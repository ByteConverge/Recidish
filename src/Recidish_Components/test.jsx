...
function SignUpClientForm() {
  ...
  const handleBlur = (e) => {
    ...
    if (name === "email" && !validateEmail(value)) {
      ...
      setErrors((prevErrors) => ({
        ...prevErrors,
        password:
          "passowrd should include numbers , letters , uppercase and lowercase alphabets",
      }));
  ...
  return (
    <form onSubmit={handleSubmit} className="signUpForm">
      {errors.form && (
        <p style={{ color: "red", fontSize: "1rem" }}>{errors.form}</p>
      )}
      {success && (
        <p style={{ color: "green", fontSize: "1.5rem", textAlign: "center" }}>
          {success}
        </p>
      )}
      {/* hidden input role */}
      ...
      />
      {errors.name && (
        <p style={{ color: "red", fontSize: "1rem" }}>{errors.name}</p>
      )}
